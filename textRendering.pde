...
HashMap<TextTriple, PImage> renderedTextCache;
Graphics2D dummyGraphicsObj;
...
void drawText(TextObject object) {
  drawText(object, 0, 0);
}

void drawText(TextObject object, int offsetX, int offsetY) {
  // Load the font.
  Font font = null;
  ...
  if (loadedFonts.containsKey(fontName)) {
    ...
    try {
      font = Font.createFont(Font.TRUETYPE_FONT, new File(sketchPath("data\\" + object.fontFamily + ".ttf")));
    } catch (Exception e) {
  ...
  image(rendered, imagePos.x, imagePos.y);
}

PImage renderTextToImageCached(String text, Font font, int maxWidth) {
...
PImage renderTextToImage(String text, Font font, int maxWidth) {
  ...
  // Convert to PImage
  PImage ret = new PImage(width, height, ARGB);
