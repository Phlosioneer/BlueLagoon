...
HexCoords pointToHex(MapFile<PImage> map, float x, float y, float mapOffsetX, float mapOffsetY) {
  return pointToHex(map, x - mapOffsetX, y - mapOffsetY);
}

HexCoords pointToHex(MapFile<PImage> map, float x, float y) {
  ...
  if (remainingHeight > pointLen) {
    ...
    boolean parity = (abs(halfColumn + upperRowNum) % 2 == 1);
...
PVector hexToCornerPoint(MapFile<PImage> map, HexCoords coords, float mapOffsetX, float mapOffsetY) {
...
PVector hexToCenterPoint(MapFile<PImage> map, HexCoords coords, float mapOffsetX, float mapOffsetY) {
...
PVector hexToCenterPoint(MapFile<PImage> map, HexCoords coords) {
  PVector ret = hexToCornerPoint(map, coords);
